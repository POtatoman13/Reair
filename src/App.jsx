...
const App = () => {
  ...
  const AGENTE_PERC = 0.07; // 7% provvigione
  ...
  const generateAI = async (type) => {
    ...
    const prompt = type === 'pitch' 
      ? `Sei un esperto ReAir. Scrivi un pitch di vendita IRRESISTIBILE (2-3 righe). Impianto ${kwp}kWp. Recupero investimento (Break-even) in soli 12 MESI. Secondo anno di protezione incluso (24 mesi totali) è puro profitto. Prezzo netto: €${calcs.nettoAlCliente.toLocaleString()}.`
  ...
  return (
    <div className={`min-h-screen font-sans transition-all ${isClientMode ? 'bg-slate-950 text-white' : 'bg-slate-50 text-slate-900'}`}>
      ...
      {/* BOLLINO INDACO V14 */}
      <div className="bg-indigo-700 text-white text-[10px] font-black text-center py-1 uppercase tracking-widest flex items-center justify-center gap-2">
        <RefreshCw className="w-3 h-3" /> 
        Versione 14.0 - Analisi Costi Operativi Integrata - Se non è indaco, non è aggiornato!
      </div>
      ...
      <nav className={`sticky top-0 z-30 border-b px-4 py-3 flex justify-between items-center ${isClientMode ? 'bg-slate-950 border-slate-800' : 'bg-white border-slate-200 shadow-sm'}`}>
        <div className="flex items-center gap-2">
          <Sun className="text-indigo-600 w-5 h-5" />
          <h1 className="text-sm font-black uppercase tracking-tighter text-inherit">ReAir <span className="text-indigo-600">Operations</span></h1>
        </div>
        <button onClick={() => setIsClientMode(!isClientMode)} className="bg-indigo-600 text-white px-5 py-2 rounded-full text-[10px] font-black uppercase shadow-lg active:scale-95 transition-all">
      ...
      <main className="max-w-4xl mx-auto p-4 space-y-6 pb-24">
        ...
        <section className={`p-6 rounded-3xl border ${isClientMode ? 'bg-slate-900 border-slate-800' : 'bg-white border-slate-200 shadow-sm'}`}>
          <div className="space-y-6 text-inherit text-slate-900">
            <div>
              <label className="text-[10px] font-black uppercase opacity-40 mb-2 block tracking-widest text-inherit">Potenza Fotovoltaico (kWp)</label>
              <input type="number" value={kwp} onChange={(e) => setKwp(Number(e.target.value))} className="w-full bg-transparent border-b-2 border-indigo-600 py-2 text-3xl font-black outline-none" />
            </div>

            <div className="grid grid-cols-2 gap-2">
              {Object.values(PRODUCTS).map(p => (
                <button key={p.id} onClick={() => setSelectedProductId(p.id)} className={`p-3 rounded-2xl border-2 text-left transition-all ${selectedProductId === p.id ? 'border-indigo-600 bg-indigo-600/10' : 'border-transparent bg-slate-100/50'}`}>
            ...
            <div className="grid grid-cols-2 gap-2 pt-2">
              ...
              <button onClick={() => setBonusFedeltaAttivo(!bonusFedeltaAttivo)} className={`p-3 rounded-xl border-2 font-black text-[9px] uppercase transition-all ${bonusFedeltaAttivo ? 'border-indigo-600 text-indigo-600 bg-indigo-600/5' : 'border-slate-200 opacity-40'}`}>
                ⭐ Bonus Fedeltà -2%
              </button>
            </div>
          </div>
        ...
        {/* AI PANEL */}
        <section className={`p-6 rounded-3xl border-2 ${isClientMode ? 'bg-slate-900 border-indigo-900' : 'bg-white border-indigo-100'}`}>
          <div className="flex justify-between items-center mb-4">
            <span className="text-[10px] font-black uppercase text-indigo-600 flex items-center gap-2"><Sparkles className="w-4 h-4 animate-pulse" /> ReAir Sales Assistant</span>
            <div className="flex gap-2">
              ...
              <button onClick={() => generateAI('esg')} disabled={isAiLoading} className="bg-slate-700 text-white text-[10px] font-black px-4 py-2 rounded-xl uppercase shadow-md active:scale-95">ESG</button>
            </div>
          </div>
          {aiContent && <div className="p-4 bg-indigo-500/5 rounded-2xl border border-indigo-500/20 text-inherit text-sm italic">"{aiContent}"</div>}
        </section>
        ...
        <div className={`rounded-[40px] overflow-hidden border shadow-2xl ${isClientMode ? 'bg-slate-900 border-slate-800' : 'bg-white border-slate-100'}`}>
          <div className="p-8 bg-indigo-700 text-white flex justify-between items-center">
            ...
            <div className="text-right bg-white/10 px-4 py-1 rounded-xl border border-white/20"><span className="text-[10px] font-black block">RE-COD</span><span className="font-bold text-sm uppercase">PROJ-{kwp}</span></div>
          </div>
          ...
          <div className="p-8 space-y-8 text-slate-900">
            <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
              <div className={`p-6 rounded-3xl border-2 text-center ${isClientMode ? 'bg-slate-950 border-slate-800' : 'bg-slate-50 border-slate-100 text-slate-900'}`}>
                <span className="text-[10px] font-black opacity-30 uppercase block mb-1">Valore Commessa</span>
              ...
              <div className="p-8 bg-emerald-600 rounded-[32px] text-white text-center shadow-xl flex flex-col justify-center transform hover:scale-[1.02] transition-transform">
                <span className="text-[10px] font-black opacity-70 uppercase block mb-1">Costo Netto d'Impresa</span>
                <span className="text-5xl font-black tracking-tighter text-white">€ {calcs.nettoAlCliente.toLocaleString(undefined, {maximumFractionDigits:0})}</span>
            ...
            <div className={`p-6 rounded-3xl border ${isClientMode ? 'bg-slate-950 border-slate-800' : 'bg-slate-50 border-slate-200'}`}>
              <div className="flex justify-between items-center mb-6">
                <span className="text-[10px] font-black uppercase opacity-40">Analisi ROI Lineare</span>
                <span className="text-indigo-600 font-black text-xs uppercase tracking-tighter underline">Pareggio al 12° Mese</span>
              </div>
              <div className="h-24 w-full relative">
                <svg className="w-full h-full overflow-visible" viewBox="0 0 400 200">
                  <line x1="0" y1="150" x2="400" y2="150" stroke="currentColor" strokeWidth="1" strokeDasharray="4" opacity="0.2" />
                  <path d="M 0 190 L 200 150 L 400 110" fill="none" stroke="#4f46e5" strokeWidth="6" strokeLinecap="round" />
                  <circle cx="200" cy="150" r="10" fill="#4f46e5" className="animate-pulse" />
                  <text x="200" y="130" textAnchor="middle" className="text-[10px] font-black fill-indigo-600 uppercase">Break-Even 12m</text>
                </svg>
              </div>
            </div>

            <div className="grid grid-cols-2 gap-4">
              <div className="text-center p-5 bg-indigo-500/5 rounded-3xl border border-indigo-500/10 text-inherit">
        ...
        {!isClientMode && (
          <div className="space-y-6 animate-in slide-in-from-bottom-4 duration-700">
            ...
            <section className="bg-slate-900 text-white p-8 rounded-[40px] border border-white/5 shadow-2xl">
              <h2 className="text-[10px] font-black uppercase opacity-50 mb-6 flex items-center gap-2 text-indigo-400"><Briefcase className="w-4 h-4" /> Partner Margin Analysis</h2>
        ...
        <div className="text-center pt-4">
          <button onClick={() => window.location.reload(true)} className="text-[9px] font-black text-slate-400 uppercase tracking-widest border border-slate-300 px-4 py-2 rounded-xl active:bg-slate-200 transition-all">
            Sincronizza Aggiornamento Costi Operativi
          </button>
        </div>
      </main>
