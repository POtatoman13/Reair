...
const MODELS = [
  "gemini-2.5-flash-preview-09-2025", // Modello primario supportato in Canvas
  "gemini-1.5-flash"
];
...
const App = () => {
  ...
  return (
    <div className={`min-h-screen transition-all duration-700 overflow-x-hidden ${isClientMode ? 'bg-white text-slate-900' : 'bg-slate-950 text-white'}`}>
      ...
      {/* CSS AVANZATO PER PDF MULTI-PAGINA */}
      <style>{`
        @media print {
          @page { size: A4; margin: 20mm 15mm; }
          nav, .no-print, button, footer { display: none !important; }
          body { background: white !important; color: #0f172a !important; -webkit-print-color-adjust: exact !important; print-color-adjust: exact !important; overflow: visible !important; position: static !important; }
          .print-container { padding: 0 !important; width: 100% !important; max-width: 100% !important; margin: 0 !important; box-shadow: none !important; border: none !important; }
          .quote-card { 
      ...
      <main className="max-w-6xl mx-auto p-4 md:p-10 space-y-8 pb-32 print-container text-inherit">
        ...
        <div className="print-header-top">
           ...
           <div className="text-right">
             <h1 className="font-black text-3xl text-[#005A8C] uppercase m-0 tracking-tighter">Proposta Tecnica ReAir</h1>
             <p className="text-[10px] font-bold text-slate-500 uppercase tracking-widest mt-1 italic">Dipartimento Nanotecnologie 2026</p>
             <p className="text-[12px] font-black text-slate-900 mt-2">Data: {new Date().toLocaleDateString('it-IT')}</p>
           </div>
        </div>
        ...
        <section className={`no-print p-10 rounded-[48px] border transition-all shadow-2xl space-y-12 ${isClientMode ? 'hidden' : 'bg-slate-900 border-white/10'}`}>
          <div className="grid grid-cols-1 lg:grid-cols-3 gap-12">
            ...
            <div className="space-y-8">
              ...
              <div className="pt-4">
                <span className="text-[11px] font-black uppercase opacity-40 block mb-4 tracking-widest">Opzione Ciclo Protocollo:</span>
        ...
        <div className={`client-box p-10 rounded-[40px] border-2 border-dashed transition-all ${isClientMode ? 'border-slate-200 bg-slate-50 text-slate-900 shadow-sm' : 'border-white/10 bg-white/5 text-white'}`}>
          <div className="flex flex-col md:flex-row justify-between items-start md:items-center gap-8">
            <div className="flex-1">
              ...
              <h2 className="text-5xl font-black uppercase tracking-tighter text-inherit leading-none">
                {customerName || "Gentile Cliente"}
              </h2>
            </div>
            <div className="bg-[#005A8C] text-white px-10 py-6 rounded-[32px] shadow-2xl border border-white/20 text-center min-w-[200px]">
              <span className="text-[10px] font-black uppercase opacity-60 block mb-1 tracking-widest">Protocollo ReAir</span>
              <p className="text-4xl font-black">{cycleMonths} MESI</p>
            </div>
          </div>
        </div>
        ...
        {/* SCHEDE PREVENTIVO (LOGICA MULTI-PAGINA) */}
        <div className="grid grid-cols-1 gap-12">
          {calcs.filter(p => activeProducts.includes(p.id)).map(p => (
            <div key={p.id} className={`quote-card p-12 rounded-[64px] border transition-all shadow-2xl flex flex-col lg:flex-row gap-16 items-center overflow-hidden relative ${isClientMode ? 'bg-white border-slate-100 text-slate-900' : 'bg-white/5 border-white/10 text-white'}`}>
              <div className="absolute top-0 right-0 w-64 h-64 bg-[#8EBCD6]/10 rounded-bl-full -z-10 opacity-50"></div>
              
              <div className="flex-1 space-y-8">
                <div>
                  <h3 className={`text-5xl font-black uppercase tracking-tighter ${isClientMode ? 'text-[#005A8C]' : 'text-blue-400'}`}>{p.name}</h3>
                  <p className="text-[11px] font-bold opacity-50 uppercase tracking-[0.5em] mt-3 tracking-widest text-inherit italic">Protocollo di Mantenimento Certificato 2026</p>
                </div>
                <p className="text-xl font-medium leading-relaxed opacity-80 text-inherit">
                ...
                <div className="flex flex-wrap gap-10 pt-6">
                  <div className="flex items-center gap-4 text-emerald-600 font-black uppercase text-[14px]"><ShieldCheck className="w-8 h-8"/> Efficacia Certificata</div>
                  <div className="flex items-center gap-4 text-blue-600 font-black uppercase text-[14px]"><TrendingUp className="w-8 h-8"/> Performance Asset</div>
                </div>
              </div>

              <div className="w-full lg:w-[480px] space-y-6">
                <div className={`p-10 rounded-[40px] border text-center transition-all ${isClientMode ? 'bg-slate-50 border-slate-100 shadow-inner' : 'bg-black/40 border-white/5'}`}>
                   <span className="text-[12px] font-black uppercase opacity-40 block mb-3 tracking-[0.3em] text-inherit">Valore del Protocollo ({cycleMonths} mesi)</span>
                   <span className="text-6xl font-black text-inherit leading-none italic">€ {p.finalPrice.toLocaleString(undefined, {maximumFractionDigits:0})}</span>
                </div>
                <div className={`p-12 rounded-[48px] text-white shadow-2xl text-center transform hover:scale-[1.02] transition-all bg-gradient-to-br ${p.category === 'outdoor' ? 'from-[#005A8C] to-[#003A6C]' : 'from-emerald-600 to-teal-800'}`}>
                   <span className="text-[12px] font-black uppercase opacity-70 block mb-3 tracking-widest text-white/90">Investimento Netto d'Impresa Reale</span>
                   <span className="text-7xl font-black text-white leading-none">€ {p.netPrice.toLocaleString(undefined, {maximumFractionDigits:0})}</span>
                   <p className="text-[11px] mt-6 font-bold opacity-90 uppercase tracking-widest text-white/80 border-t border-white/20 pt-4">
                     {cycleMonths === 48 ? 'Sconto Anticipo 15% Incluso + Detrazione 50%' : 'Agevolazione Fiscale 50% Inclusa'}
                   </p>
                </div>
              </div>
            </div>
        ...
        {/* DASHBOARD PARTNER - ANALISI COSTI (Nascondere in Vista Cliente) */}
        {!isClientMode && (
          <section className="no-print bg-slate-900 p-12 rounded-[64px] border border-white/10 shadow-2xl space-y-12 text-white relative overflow-hidden">
            ...
            <div className="flex justify-between items-center border-b border-white/5 pb-12">
              <h2 className="text-[18px] font-black uppercase text-blue-400 tracking-[0.5em] flex items-center gap-6"><Calculator className="w-10 h-10" /> Analisi Margini Business V33.8</h2>
              <div className="text-right">
            ...
            <div className="overflow-x-auto">
              <table className="w-full text-left text-[15px]">
                <thead className="opacity-40 uppercase font-black text-white tracking-[0.3em] border-b border-white/5">
                  <tr>
                    <th className="pb-12">Tecnologia Soluzione</th>
                    <th className="pb-12 text-center">Status</th>
                    <th className="pb-12">Costo Materiali</th>
                    <th className="pb-12">Provv. Agente</th>
                    <th className="pb-12 text-right">Utile Soci (40%)</th>
                  </tr>
                </thead>
                <tbody className="font-bold text-white">
                  {calcs.map(p => (
                    <tr key={p.id} className={`border-t border-white/5 group hover:bg-white/5 transition-colors ${!activeProducts.includes(p.id) ? 'opacity-10' : ''}`}>
                      ...
                      <td className="py-12 text-center italic">
                        {activeProducts.includes(p.id) ? 'ATTIVO' : 'ESCLUSO'}
                      </td>
                      <td className="py-12 text-xl font-black tracking-tight">€ {p.matCost.toLocaleString(undefined, {maximumFractionDigits:0})}</td>
                      <td className="py-12 text-orange-400 text-xl font-black tracking-tight">€ {p.comm.toLocaleString(undefined, {maximumFractionDigits:0})}</td>
                      <td className="py-12 text-emerald-400 text-right text-4xl font-black tracking-tighter">€ {p.profit.toLocaleString(undefined, {maximumFractionDigits:0})}</td>
                    </tr>
                  ))}
                </tbody>
              </table>
