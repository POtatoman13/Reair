...
const App = () => {
  ...
  // --- PROPORZIONE REALE PANNELLI 455Wp ---
  // Un pannello da 455Wp è lungo circa 211cm e largo 105cm = ~2.21 mq.
  // 1 kWp = 2.19 pannelli -> Area = 2.19 * 2.21 = ~4.84 mq per kWp
  ...
  // Specchietto Tariffe Pulizia Pre-Trattamento (€/kWp)
  // Tariffe ricalibrate per "punto di incontro"
  ...
  const calcs = useMemo(() => {
    return PRODUCTS_DATA.map(p => {
      ...
      let cleaningRate = 0;
      const applicationRate = 10; // 10€/mq fissi per applicazione nano
      ...
      // Costi Vivi (Materiali + Manodopera Totale)
      ...
      // Calcolo Prezzo Finale: garantisce 40% utile soci e 7% agente (Divisore 0.53)
  ...
  const generateAI = async (mode) => {
    ...
    const prompt = `Agisci come Direttore ReAir. Fornisci un ${mode} professionale. Nota tecnica: Proporzione pannelli 455Wp (~4.84mq/kWp). Costo applicazione: 10€/mq. Sopralluogo 12° mese incluso. Niente 'garanzia'.`;
    ...
    try {
      ...
      setAiResponse("Errore connessione.");
    } finally {
      setIsLoading(false);
    }
  };
  ...
  return (
    <div className={`min-h-screen transition-all duration-700 overflow-x-hidden ${isClientMode ? 'bg-white text-slate-900' : 'bg-slate-950 text-white'}`}>
      ...
      <style>{`
        @media print {
          ...
          .quote-card { 
            ...
            margin-bottom: 30px !important; background: white !important; border-radius: 20px !important; padding: 25px !important; width: 100% !important;
          }
      ...
      <div className="no-print text-white text-[9px] font-black text-center py-1.5 uppercase tracking-[0.5em] flex items-center justify-center gap-2 bg-[#005A8C]">
        <ShieldCheck className="w-3 h-3" /> ReAir Quote Engine V34.8 - Optimized Ratios
      </div>
          ...
          <section className="no-print p-8 rounded-[48px] border bg-slate-900 border-white/10 shadow-2xl space-y-12 text-white">
            <div className="grid grid-cols-1 lg:grid-cols-3 gap-12">
              <div className="space-y-8">
                ...
                <div>
                  <span className="text-[11px] font-black uppercase opacity-40 block mb-4 tracking-widest">Durata Protocollo:</span>
              ...
              <div className="space-y-8">
                ...
                <div className="pt-4">
                  ...
                  <p className="text-[9px] font-bold opacity-30 uppercase mt-4">Rapporto: 1 kWp (pannelli 455W) = ~{MQ_PER_KW} mq</p>
                </div>
              </div>
            </div>
          </section>
        ...
        {!isClientMode && (
          <section className="no-print bg-slate-900 p-12 rounded-[64px] border border-white/10 shadow-2xl space-y-12 text-white relative overflow-hidden">
            ...
            <div className="flex justify-between items-center border-b border-white/5 pb-12 text-inherit">
              <h2 className="text-[18px] font-black uppercase text-blue-400 tracking-[0.5em] flex items-center gap-6 text-inherit"><Calculator className="w-10 h-10" /> Business Analytics V34.8</h2>
              <div className="text-right text-inherit">
                <span className="text-3xl font-black text-orange-400">7.0% Net Fee Agente</span>
              </div>
            </div>
              ...
              <table className="w-full text-left text-[14px] text-inherit">
                <thead className="opacity-40 uppercase font-black text-white tracking-[0.3em] border-b border-white/5 text-inherit">
                  <tr>
                    ...
                    <th className="pb-12 text-inherit text-center">Posa (Pulizia + Appl)</th>
                  ...
                  {calcs.map(p => (
                    <tr key={p.id} className={`border-t border-white/5 group hover:bg-white/5 transition-colors ${!activeProducts.includes(p.id) ? 'opacity-10' : ''}`}>
                      ...
                      <td className="py-12 text-xl font-black text-center text-inherit">
                        ...
                        <div className="text-[9px] font-bold opacity-30 mt-2 uppercase text-inherit">
                          {p.category === 'outdoor' ? `${p.cleaningRate}€/kW + 10€/mq (@ ~${p.mq.toFixed(0)}mq)` : '10€/mq'}
                        </div>
                      </td>
                      <td className="py-12 text-orange-400 text-xl font-black text-inherit">€ {p.comm.toLocaleString(undefined, {maximumFractionDigits:0})}</td>
